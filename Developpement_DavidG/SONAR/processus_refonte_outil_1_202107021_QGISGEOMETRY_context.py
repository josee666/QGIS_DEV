...
def grilleSondage(us, reptrav,x, y):
    ...
    processing.run("native:creategrid", {'TYPE':2,'EXTENT':coords,
                                        ...
                                        'OUTPUT':'ogr:dbname=\'{0}\' table=\"grille\" (geom) sql='.format(gpkg)})
    ...
    grille_tuiles_tmp = processing.run("native:extractbylocation", {'INPUT':grille,
                                                                    'PREDICATE':[0],'INTERSECT':us,
                                                                    'OUTPUT': QgsProcessing.TEMPORARY_OUTPUT},context=context)["OUTPUT"]
    ...
    processing.run("native:extractbylocation", {'INPUT':grille_placettes_tmp,
                                                'PREDICATE':[0],'INTERSECT':us,
                                                'OUTPUT':'ogr:dbname=\'{0}\' table=\"grille_placettes_tmp2\" (geom) sql='.format(gpkg)},context=context)
    ...
    return gpkg,grille_tuiles, grille_placettes,us,ESPG
...
def main():
    ...
    processing.run("native:joinattributesbylocation", {'INPUT':grille_placettes,'JOIN':path_PeupForestiers,
                                                       'PREDICATE':[0],
    ...
    # processing.run("native:createspatialindex", {'INPUT':path_PentesNum})

    processing.run("native:joinattributesbylocation", {'INPUT':df_placettes_metadata_PeupForestiers_US,'JOIN':path_PentesNum,
                                                       'PREDICATE':[0],
    ...
    # Le join avec GEOPANDAS est plus lent que QGIS
    # supprimerUnChamp(df_placettes_metadata_PeupForestiers_US, "Shape_Leng")
    # supprimerUnChamp(df_placettes_metadata_PeupForestiers_US, "Shape_Area")
    # df = gpd.read_file(gpkg, layer="df_placettes_metadata_PeupForestiers_US")
    # pente = gpd.read_file(path_PentesNum)
    # joindfPente = gpd.sjoin(df,pente, how="inner", op='intersects')
    # joindfPente.to_file(gpkg, layer="df_placettes_metadata_PeupForestiersPenteNum", driver="GPKG")
    ...
    # Selectionne de toutes les entité
    layerdf_placettes_metadata_PeupForestiersPenteNum.selectAll()
    ...
    # print("aipf")
    # Désuet
    # #Peuplement nonAIPF. Je vais utiliser les peuplement NON AIPF pour les exclures du plan de sondage.
    # processing.run("qgis:selectbyexpression", {'INPUT':layerdf_placettes_metadata_PeupForestiersPenteNum,
    ...
    # TODO a partir d'ici je pourrais faire une boucle avec mes buffers.
   ...
   #  # TODO avec cette requete je n'ai pas a faire celle de R qui correcpond a : Va chercher la combinaison origines non égal à NA et ET1_haut égal à NA
    ...
    lyrpath_PeupForestiersFix = QgsVectorLayer(path_PeupForestiers, 'lypath_PeupForestiersFix', 'ogr')
    ET1_HAUT_NUM= QgsField('ET1_HAUT_NUM', QVariant.Int, '3')
   ...
   #  Liste de tous les buffers dans le geopackage
    ...
    #TODO  Buffer_in,path_SIP sont des couches fournies. Il n'y a pas de buffer a faire. Donc je vais l'ajouter dasn ma liste du merge final du buffer
    # listBuf.append(path_SIP)
    listBuf.append(path_BufferIn)

    # processing.run("native:createspatialindex", {'INPUT':df_placettes_metadata_PeupForestiersPenteNum})
    ...
    for li in listBuf:
        ...
        # processing.run("native:createspatialindex", {'INPUT':li})
    ...
    listChampAsupprimer = ["fid_2","ORIG_FID", "JOIN_FID", "CO_IMP", "IPF_ZMI", "IPF_UFZ", "MODE_GEST",
                           ...
                           "INDICATIF", "ID_GEO", "SHAPE_Leng_2","ET1_HAUT_NUM", "CL_PENT2"]
