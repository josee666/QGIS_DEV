...
#
#     """
#         Permet de calculer les valuers d'un champ avec les valeurs d'un autre champ avec ou sans selection
#
#
#              Args:
#                  layer : QGS layer
...
def main():
    ...
    # processing.run("native:fieldcalculator", {'INPUT':layerdf_placettes_metadata_PeupForestiersPenteNum,'FIELD_NAME':'ET1_HAUT_NUM',
    #                                           'FIELD_TYPE':0,
    #                                           'FIELD_LENGTH':3,
    #                                           'FIELD_PRECISION':0,'FORMULA':' \"ET1_HAUT\" ',
    #                                           'OUTPUT':'ogr:dbname=\'{0}\' table=\"df_placettes_metadata_PeupForestiersPenteNumTMP\" (geom) sql='.format(gpkg)})
    #
    # # Effacer la couche df_placettes_metadata_PeupForestiersPenteNum pour la remplacer par df_placettes_metadata_PeupForestiersPenteNumTMP
    # CREATE_NO_WINDOW = 0x08000000
    # cmd = r"""ogrinfo {0} -sql "drop table df_placettes_metadata_PeupForestiersPenteNum""".format(gpkg)
    # subprocess.call(cmd, creationflags=CREATE_NO_WINDOW)
    #
    # CREATE_NO_WINDOW = 0x08000000
    # cmd = r"""ogrinfo {0} -sql "ALTER TABLE df_placettes_metadata_PeupForestiersPenteNumTMP RENAME TO df_placettes_metadata_PeupForestiersPenteNum""".format(gpkg)
    # subprocess.call(cmd, creationflags=CREATE_NO_WINDOW)
   ...
   #  # TODO avec cette requete je n'ai pas a faire celle de R qui correcpond a : Va chercher la combinaison origines non égal à NA et ET1_haut égal à NA
    ...
    # processing.run("native:fieldcalculator", {'INPUT':lyrpath_PeupForestiersFix,'FIELD_NAME':'ET1_HAUT_NUM',
    #                                           'FIELD_TYPE':0,
    #                                           'FIELD_LENGTH':3,
    #                                           'FIELD_PRECISION':0,'FORMULA':' \"ET1_HAUT\" ',
    #                                           'OUTPUT':'ogr:dbname=\'{0}\' table=\"PeupForestiersFixTMP\" (geom) sql='.format(gpkg)})
    #
    #
    # # Effacer la couche PeupForestiersFix pour la remplacer par PeupForestiersFixTMP
    # CREATE_NO_WINDOW = 0x08000000
    # cmd = r"""ogrinfo {0} -sql "drop table PeupForestiersFix""".format(gpkg)
    # subprocess.call(cmd, creationflags=CREATE_NO_WINDOW)
    #
    # CREATE_NO_WINDOW = 0x08000000
    # cmd = r"""ogrinfo {0} -sql "ALTER TABLE PeupForestiersFixTMP RENAME TO PeupForestiersFix""".format(gpkg)
    # subprocess.call(cmd, creationflags=CREATE_NO_WINDOW)
