...
def grilleSondage(us, reptrav,x, y):
    ...
    champ = QgsField('tuile_ID', QVariant.String, '10')
...
class CreationplacetteMesurableAlgorithm(QgsProcessingAlgorithm):
    """
    ...
    def initAlgorithm(self, config):
        ...
        self.addParameter(QgsProcessingParameterFile
                          (self.Dossier,'Dossier des intrants',
                                                     behavior=QgsProcessingParameterFile.Folder,
                                                     defaultValue=None))

        # parametre de la prise de vue
        self.addParameter(
            QgsProcessingParameterNumber(
                self.anneePhoto,
                ('Année de la prise de vue de la photo'),
                defaultValue=2012,
                optional=False,
                minValue=2012,
                maxValue=2030))
        ...
        # parametre de la grile en X
        self.addParameter(
            QgsProcessingParameterString(
                self.grilleX,
                ('Dimension de la grille en X (m)'),
                defaultValue=1000,
                optional=False)
        )

        # parametre de la grile en Y
        self.addParameter(
            QgsProcessingParameterString(
                self.grilleY,
                ('Dimension de la grille en Y (m)'),
                defaultValue=1000,
                optional=False)
        )
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        # Parametres Dossier
        wd_intrants = self.parameterAsString(parameters,  self.Dossier, context)
        ...
        # anne de prise de vue
        annee_photo = self.parameterAsInt(parameters, self.anneePhoto, context)

        # numéro de l'unité de sondage
        us = self.parameterAsString(parameters, self.uniteSondage, context)

        # dimension de la grille en X
        x = self.parameterAsInt(parameters, self.grilleX, context)

        # dimension de la grille en Y
        y = self.parameterAsInt(parameters, self.grilleY, context)
        ...
        processing.run("native:joinattributesbylocation", {'INPUT':grille_placettes,'JOIN':path_PeupForestiersFix,
                                                           ...
                                                           'NON_MATCHING':'TEMPORARY_OUTPUT'},context=noCheckGeom)
        ...
        feedback.pushInfo("\n6. Calculs des placettes exclues (Buffer)\n")
        ...
        listChampAsupprimer = ["fid_2","ORIG_FID", "JOIN_FID", "CO_IMP", "IPF_ZMI", "IPF_UFZ", "MODE_GEST",
                               ...
                               "Shape_Leng","Shape_Area","ET1_HAUT_NUM","PR_IMP", "fid_3", "CL_PENT_2"]
        ...
        # Copier le geopackage dans le dossier Extrants
        dossierExtrants = wd_intrants.strip().split('\\')
        del (dossierExtrants[-1])
        dossierExtrants = '\\'.join(dossierExtrants)
        dossierExtrants = os.path.join(dossierExtrants,"Extrants")

        if not os.path.exists(dossierExtrants):
            os.makedirs(dossierExtrants)

        shutil.copy(gpkg,os.path.join(dossierExtrants, "SONAR2_outil_1.gpkg"))


        gpkg = os.path.join(dossierExtrants, "SONAR2_outil_1.gpkg")
        ...
        feedback.pushInfo("\n Les résultats sont dans le Geopackage SONAR2_outil_1.gpkg dans le dossier {0}\n".format(dossierExtrants))
    ...
    def name(self):
        ...
        """
        return '1 - Création de placettes mesurables'

    def icon(self):
        return QIcon(os.path.dirname(__file__) + '/image/Mesurable.jpg')
    ...
    def createInstance(self):
        return CreationplacetteMesurableAlgorithm()
